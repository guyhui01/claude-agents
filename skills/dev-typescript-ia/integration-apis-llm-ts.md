...
- ❌ **Hardcoded model** scattered around: centralize the ID (`claude-opus-5-5`) in config
- ❌ **`content[0]` assumed to be `text`** without checking the type: exception on a block-based response → type guard (already present ✓)

## Sources
- **Anthropic API** — docs.anthropic.com (Messages API, **prompt caching**: cache read ≈ 0.1× the input price, TTL 5 min) · current model **`claude-opus-5-5`**
