...
> ⚠️ **Educational vs. Production**: this skill presents **reference patterns with explicit stubs** to complete per use case. The `generateUserStories()`, `prioritizeBacklog()`, etc. functions are extension points to implement — either via an Anthropic SDK call or via your own business logic. Real production: see `skills/orchestrateur-workflow/claude-api-integration.md` for a complete, working Anthropic SDK integration.
...
// See skills/orchestrateur-workflow/claude-api-integration.md section "Tool Use"
```
...
- [`../dev-typescript-ia/mcp-server-dev.md`](../dev-typescript-ia/mcp-server-dev.md) — developing an MCP server
