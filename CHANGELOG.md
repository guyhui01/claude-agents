...
### 🐛 Fixed
- **`validate:wf-agents` printed a green over an empty corpus.** With the ten `WF-*.md` identity cards gone, `deriveExpected()` returned an empty map, the comparison loop iterated over nothing, and the guard reported `✓ "Agents" column consistent: 0 workflow(s) × 2 file(s)` and exited **0**. Its sibling `validate:skill-mapping` had carried an empty-corpus guard since 2026-08-09; this one never did — two sibling guards settling the same policy in opposite directions, each green on its own fixture. It now exits 1 with its own message.
- **`generate-sidecar.mjs` had no corpus floor**, which is what turned that vacuous green into a catalog-wide failure. Measured end to end: remove the ten WF cards, run `npm run generate:sidecar`, and the generator rewrites `sidecar.json` from 85 down to **75 assets** — after which **all four gates report green** while ten workflows have silently left the catalog and the published "85 assets, all indexed" claim has gone false. `assertCorpusNonEmpty` now refuses to generate when agents, skills or workflows is empty. A floor of one catches 10 → 0; it does **not** catch 10 → 3, and a frozen expected count would break every legitimate addition, so it is deliberately not used.
- **Missing corpus directories now fail with the tool's own message instead of a raw ENOENT stack** (`check-skill-mapping.mjs`, `check-workflow-agent-counts.mjs`, and both `readdirSync` call sites in `generate-sidecar.mjs`). The verdict was already correct — Node exits 1 on an uncaught throw, measured — so this is diagnosis, not correctness. It has one real consequence: the ENOENT throw made `check-skill-mapping`'s empty-corpus report **unreachable**, so that guard's 2026-08-09 red proof had only ever covered the *present-but-empty* shape, never the *absent* one.
- **`package-lock.json` kept naming the previous version, and nothing measured it.** `npm ci` accepts a stale root version (measured: exit 0), so the drift recurred silently: of the **9** tags carrying both files, **5** shipped with a lock still on the previous version — `v3.27.0`, `v3.27.2`, `v3.27.3`, `v4.1.0`, and this release before the fix. `catalogTag()` now compares `package.json` against both `version` and `packages[""].version` in the lock and refuses to generate on a mismatch, so the 1st CI gate carries it — no new gate, no new CI wiring. Every asset is stamped with this tag, so a disagreement would publish a `catalogVersion` contradicting the package identity it derives from. Proven red on four causes (each lock field alone, the release gesture `package.json` bumped first, and a missing lock).
- Every guard was proven **red on its own cause** before being trusted green, and *absent* / *present-but-empty* / *populated* now print **three distinct outcomes** in all three tools — "did not run", "nothing to measure" and "passed" must never render the same green.
...
### 📝 Documentation
- **"Four green gates" is precise locally and loose in CI.** `check:schema-drift` compares the vendored schema against the runtime's; the runtime is not checked out in the `sidecar` job, so the step prints `✓ identity pin OK` followed by `⚠ drift-check skipped: runtime schema not found` and exits **0** — by design, and stated as such in `.github/workflows/sidecar.yml`. In CI **three** gates actually measure the catalog and the fourth verifies only the identity pin; locally all four measure. The trackers now say which.
...
## [4.2.0] — 2026-07-18 — Workflows indexed in the sidecar (85 assets)
